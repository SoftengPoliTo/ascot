...
impl Discovery {
    ...
    /// Discovers all available devices in a network and returns them.
    ///
    /// # Errors
    ///
    /// ## Discovery Errors
    ...
    ///
    /// When a request is sent with the objective of retrieving information
    /// about a device, some of the most common errors which prevent the
    /// returned response to be received correctly might be network timeouts or
    /// failures during a response parsing.
    pub async fn discover(&self) -> Result<Devices, Error> {
        // Discover devices.
                ...
                match reqwest::get(&complete_address).await {
                    Ok(response) => {
                        ...
                        let requests = device_data
                            ...
                            .collect::<HashMap<String, RequestSender>>();
                        ...
                        devices.add(Device::new(network_info, description, requests));
...
mod tests {
    ...
    use crate::tests::{compare_device_data, light_with_toggle, light_without_toggle, DOMAIN};

    use super::Discovery;

    async fn discovery_comparison(devices_len: usize) {
        let devices = Discovery::new(DOMAIN)
            .timeout(Duration::from_secs(1))
            .disable_ipv6()
            .disable_ip(Ipv4Addr::new(172, 17, 0, 1))
            .discover()
            .await
            .expect("Device discovery failed.");

        // The devices collection MUST contain only a single device.
    ...
    async fn test_more_devices_discovery() {
        ...
        let (close_tx1, close_rx1) = tokio::sync::oneshot::channel();
        let (close_tx2, close_rx2) = tokio::sync::oneshot::channel();

        // Run first device task.
        let device1_handle = tokio::spawn(async { light_without_toggle(close_rx1).await });

        // Wait for first device task to be configured.
        tokio::time::sleep(Duration::from_millis(100)).await;

        // Run second device task.
        let device2_handle = tokio::spawn(async { light_with_toggle(close_rx2).await });
